...
@MainActor
final class RequestLogsWindowController: NSObject, NSWindowDelegate {
    ...
    func showWindow() {
        ...
        window.title = model.text(.requestLogsTitle)
        window.styleMask = [.titled, .closable, .miniaturizable, .resizable]
        window.minSize = NSSize(width: 1280, height: 840)
    ...
    func closeWindow() {
        if let window {
            ...
            self.model?.handleRequestLogsWindowDidClose()
        }
    }
    ...
    func refreshWindow() {
        ...
        self.window?.title = model.text(.requestLogsTitle)
